...
_(Pro Tip: If you are spinning up a fresh database to test this, using the official `kartoza/postgis` Docker image comes with many of these spatial optimisations ready to go.)_
...
Never guess why a query is slow; ask the database directly. By putting `EXPLAIN (ANALYZE, BUFFERS)` before your `SELECT` statement, PostgreSQL will output the exact execution plan. _(Note: The SQL command itself retains the American spelling `ANALYZE`, as it is hardcoded into PostgreSQL.)_
...
2. Run an `EXPLAIN (ANALYZE, BUFFERS)` to find all points within a specific bounding box _before_ indexing:
...
3. Now, create a GiST index on the `geom` column and run `ANALYZE test_points;`.
4. Run the same `EXPLAIN (ANALYZE, BUFFERS)` query again.
