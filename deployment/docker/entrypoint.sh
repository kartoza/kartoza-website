...
GATEWAY_ERPNEXT_ISSUE_API_URL="${GATEWAY_ERPNEXT_ISSUE_API_URL:-https://api.do.kartoza.com/erpnext/kartoza-website/api/method/kartoza_custom.api.create_support_ticket}"
...
export gateway_erpnext_issue_api_url="${GATEWAY_ERPNEXT_ISSUE_API_URL}"

# Create temporary config with substituted variables in writable location
envsubst '${erpnext_api_auth_base64} ${gateway_erpnext_api_url} ${gateway_erpnext_issue_api_url}' < /etc/nginx/templates/default.conf.template > /tmp/nginx/conf.d/default.conf
